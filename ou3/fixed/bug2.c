...
int main(void){
	...
	failed = 0; 
	tested = 0; 
	for (i=0 ; i<100 ; i++) {	// added 3 pairs of missing brackets
		for(j=0 ; j<10 ; j++) {					
			if (mybuffer[i][j] != rand()) {	  // fixed if-statement
				printf("Random number mismatch!\n"); 
				failed = 1;
			}
			tested++; 		// let tested be incremented
		} 
	}    
	if (failed)
	...
	free(mybuffer); 
	return 0;	// Changed to successful return 
}
